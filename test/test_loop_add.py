...
def main():
    ...
    # **前30天的配置直接从JSON读取，不进行枚举**
    # 前30天使用配置文件中读取的cost_method和bandwidth配置
    cost_method_combination = [node['cost_method'] for node in setting['nodes']]  # 从配置中读取
    bandwidth_option_combination = [node['bandwidth'] for node in setting['nodes']]  # 从配置中读取

    # 初始化前30天的模拟，不进行枚举
    last_total_costs = []
    for _ in range(1):  # 只运行一次模拟
        global_data["total_cost"].clear()  # Clear the previous results
        last_total_cost = run_simulation(setting, cost_method_combination, bandwidth_option_combination)
    ...
    print(f"Fixed Configuration (First 30 days) - Avg Cost: {avg_cost}, Variance: {variance_cost}")
    ...
    bandwidth_options = [1024, 2048]

    # Generate all possible bandwidth combinations
    bandwidth_option_combinations = list(product(bandwidth_options, repeat=node_count))

    all_results = []

    # Run simulation for each combination of cost_method and bandwidth (only after 30 days)
    for cost_combination in cost_method_combinations:
        for bandwidth_combination in bandwidth_option_combinations:
            last_total_costs = []
            for _ in range(5):  # Run simulation 5 times for each combination
                global_data["total_cost"].clear()  # Clear the previous results
